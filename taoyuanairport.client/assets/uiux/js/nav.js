...
//ray
function tmpthird(id) 
{
  let li = document.getElementById(id);

  const divElements = li.children;

  for (let i = 0; i < divElements.length; i++) {
    const tmp1 = divElements[i];

    
    if(tmp1.classList.contains("nav-third"))
    {
      tmp1.classList.remove("nav-third");
      tmp1.classList.add("nav-third1");
    }
    else
    {
      if(tmp1.classList.contains("nav-third1"))
      {
        tmp1.classList.remove("nav-third1");
      tmp1.classList.add("nav-third");
      }
    }
  }
/*
